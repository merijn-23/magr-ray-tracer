...
void KeyEventCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	//	}
	//}
	//else if ( action == GLFW_REPEAT ) { if ( app ) if ( key >= 0 )  app->KeyRepeat( key ); }
...
float3* LoadImageF(const char* file, int& w, int& h, int& c)
{
	...
	for (int i = 0; i < s; i++)
	{
		result[i].x = data[i * c + 0];
		result[i].y = data[i * c + 1];
		result[i].z = data[i * c + 2];
	}
...
{
	int s = w * h;
	uchar* img = new uchar[3 * s];
	for (int i = 0; i < s; i++)
	{
		if(data[i].x > 1) data[i].x = 1;	
		if(data[i].y > 1) data[i].y = 1;	
		if(data[i].z > 1) data[i].z = 1;	
		img[3 * i + 0] = (uchar)(data[i].x * 255);
		img[3 * i + 1] = (uchar)(data[i].y * 255);
		img[3 * i + 2] = (uchar)(data[i].z * 255);
	}

	stbi_write_png(file, w, h, 3, img, 0);
}
